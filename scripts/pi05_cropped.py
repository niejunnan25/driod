    ...
    while True:
        ...
        for t_step in bar:
            ...
            try:
                ...
                # 当前保存的视频，是已经进行了左右裁切的视频
                # video.append(curr_obs[f"{args.external_camera}_image"])
                if curr_obs["left_image"] is not None:
                    third_video.append(curr_obs["left_image"])

                # if curr_obs["right_image"] is not None:
                #     wrist_right_video.append(curr_obs["right_image"])

                if curr_obs["wrist_image"] is not None:
                    wrist_left_video.append(curr_obs["wrist_image"])
        ...
        while not isinstance(success, float):
            success = input(
                "这次测试成功了吗？(如果成功输入 y, 失败输入 n), 或是输入一个 0~100 的数值\n"
            )
            if success == "y":
                success = 1.0
            elif success == "n":
        ...
        if input("再进行一次测试？(输入 y 或 n) ").lower() != "y":
            break
